...
void initCL(cl_device_id *device, cl_context *context, cl_program *program)
{
    ...
    *program = clCreateProgramWithSource(*context, 1, &source, NULL, NULL);
	if(*program == 0){
	printf("create program with source failed\n");
}
...
if (status != CL_SUCCESS){
    ...
    char *buff_erro =(char*) malloc(100000*sizeof(char));

    size_t log_size;
    status = clGetProgramBuildInfo(*program, *device, CL_PROGRAM_BUILD_LOG, 0, NULL,&log_size);
    char* log = (char*)malloc(sizeof(log_size));
    status = clGetProgramBuildInfo(*program, *device, CL_PROGRAM_BUILD_LOG, log_size,log,NULL);
    //status = clGetProgramBuildInfo(*program, *device, CL_PROGRAM_BUILD_LOG, 1000*sizeof(char), buff_erro, NULL);
    if (!status) {
