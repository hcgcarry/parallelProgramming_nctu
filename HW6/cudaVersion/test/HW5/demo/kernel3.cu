...
__global__ void mandelKernel(float lowerX,float lowerY,int* d_img,int resX,int resY,float stepX,float stepY,int maxIterations,size_t pitchSize,int group_size_x,int group_size_y) {
    // To avoid error caused by the floating number, use the following pseudo code
for(int x_group_index=0;x_group_index<group_size_x;x_group_index++){
    int thisX = blockIdx.x * blockDim.x + threadIdx.x+ x_group_index*(resX /group_size_x);
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    dim3 numBlock(resX / (BLOCK_SIZE_X * group_size_x), resY /( BLOCK_SIZE_Y*group_size_y));
    ...
    mandelKernel<<<numBlock, blockSize>>>(lowerX,lowerY,d_img,resX,resY,stepX,stepY,maxIterations,pitchSize/sizeof(int),group_size_x,group_size_y);
