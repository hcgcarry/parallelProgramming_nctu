...
extern double test1(float* __restrict a, float* __restrict b, float* __restrict c,int N) ;
...
int main(int argc, char * argv[]) {
  ...
  int try_num = 10;
  vector<double> time_list(try_num,0);
  switch (whichTestToRun) {
    case 1: {
      for(int i=0;i<try_num;i++){
          time_list[i] = test1(values1, values2, output, N);
      }
      sort(time_list.begin(),time_list.end());
      cout << "median of exe time" <<  time_list[try_num/2] << endl;
    }
